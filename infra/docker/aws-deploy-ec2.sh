...
create_iam_role() {
  ...
  # Ensure the tag is present even if the role already existed from a prior run
  aws iam tag-role \
    --role-name "$IAM_ROLE_NAME" \
    --tags "Key=$TAG_KEY,Value=$TAG_VALUE" \
    > /dev/null

  # Note: inline role policies (put-role-policy, below) do not support tags in AWS.
...
EOF
  ...
  # Ensure the tag is present even if the instance profile already existed from a prior run
...
# Tag an SSM parameter. Done as a separate call because `put-parameter` rejects
# `--tags` when combined with `--overwrite`.
tag_ssm_parameter() {
  local param_name="$1"
  aws ssm add-tags-to-resource \
    ...
    --tags "Key=$TAG_KEY,Value=$TAG_VALUE" \
    --region "$AWS_REGION" \
    > /dev/null 2>&1 || log_warning "Could not tag SSM parameter $param_name"
}
...
ensure_key_pair() {
  ...
  # Ensure the tag is present even if the key pair already existed from a prior run
  KEY_PAIR_ID=$(aws ec2 describe-key-pairs --key-names "$KEY_NAME" --region "$AWS_REGION" --query 'KeyPairs[0].KeyPairId' --output text)
  aws ec2 create-tags --resources "$KEY_PAIR_ID" --tags "Key=$TAG_KEY,Value=$TAG_VALUE" --region "$AWS_REGION" > /dev/null
...
create_security_group() {
  ...
  # Ensure the tag is present even if the security group already existed from a prior run
  aws ec2 create-tags --resources "$SG_ID" --tags "Key=$TAG_KEY,Value=$TAG_VALUE" --region "$AWS_REGION" > /dev/null
...
launch_instance() {
  ...
  INSTANCE_ID=$(aws ec2 run-instances \
    ...
    --tag-specifications "ResourceType=instance,Tags=[{Key=Name,Value=$INSTANCE_NAME},{Key=$TAG_KEY,Value=$TAG_VALUE}]" "ResourceType=volume,Tags=[{Key=Name,Value=$INSTANCE_NAME},{Key=$TAG_KEY,Value=$TAG_VALUE}]" \
    --region "$AWS_REGION" \
